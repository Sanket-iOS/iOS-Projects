...
@synthesize checkbox,RecurringBox,ReminDateBox,ReminImageView,PriceBox,AddBox,TimeBox,DetailBox,headingBox,blankeError1,blankeError2,blankeError3,blankeError4,blankeError5;
NSString *str;
...
- (IBAction)RecureClick:(id)sender {
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    NSInteger RID = [rid integerValue];
    ...
    if (buttonIndex == 1)
    {
        if([checkbox.currentImage isEqual:[UIImage imageNamed:@"checkbox-checked.png"]])
            ...
            Notified=false;
        NSString *UpdateQuery=[[NSString alloc]initWithFormat:@"UPDATE Reminder set Rdate='%@',Rtime='%@',Heading='%@',Price='%@',Details='%@',Addto='%@',Recurring='%@', Notified='%i' where Rid='%d'",ReminDateBox.text,TimeBox.text,headingBox.text, PriceBox.text,DetailBox.text,AddBox.text, RecurringBox.text,Notified, RID] ;
    ...
    if(buttonIndex==2)
    {
        ...
        NSString *insQuery=[[NSString alloc]initWithFormat:@"insert into Reminder (Rdate,Rtime,Heading,Price,Details,Addto,Recurring,Notified,Rid)values('%@','%@','%@',%d,'%@','%@','%@','%i',%d)",ReminDateBox.text,TimeBox.text,headingBox.text,[PriceBox.text intValue],DetailBox.text,AddBox.text,RecurringBox.text,Notified,RID];
        ...
        NSString *strQuery=[[NSString alloc]initWithFormat:@"select Rdate,Rtime,Addto,Recurring,Price from Reminder where Rid=%d",RID];
  ...
  //      NSString *strRecure=[[Notifiedarr objectAtIndex:0]objectForKey:@"RECURE"];
        ...
        [dateComponent setDay:[[dateArr objectAtIndex:2] intValue]];
        [dateComponent setMonth:[[dateArr objectAtIndex:1] intValue]];
        [dateComponent setYear:[[dateArr objectAtIndex:0] intValue]];
        
        NSArray *timeArr = [strTime componentsSeparatedByString:@":"];
        [dateComponent setHour:[[timeArr objectAtIndex:0]intValue]];
        [dateComponent setMinute:[[timeArr objectAtIndex:1]intValue]];
    ...
    ReminDateBox.text=nil;
    TimeBox.text=nil;
    PriceBox.text=nil;
    headingBox.text=nil;
    DetailBox.text=nil;
    AddBox.text=nil;
    RecurringBox.text=nil;
...
- (IBAction)GetReminderId:(id)sender {

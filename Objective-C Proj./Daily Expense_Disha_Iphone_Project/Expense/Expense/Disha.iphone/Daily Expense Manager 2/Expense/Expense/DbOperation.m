...
{
    NSString *strFromPath=[[NSBundle mainBundle]pathForResource:@"DailyManage" ofType:@"sqlite"];
    NSString *strToPath=[[self DocumentDirectoryPath]stringByAppendingPathComponent:@"DailyManage.sqlite"];
    NSLog(@"strtopath....");
    NSLog(@"%@",strToPath);
...
{
    BOOL status=NO;
    NSString *strDbFile=[[self DocumentDirectoryPath]stringByAppendingPathComponent:@"DailyManage.sqlite"];
    sqlite3 *database;
...
-(NSMutableArray *)selectTrans
{
    ...
    NSString *strDatabaseFile=[[self DocumentDirectoryPath] stringByAppendingPathComponent:@"DailyManage.sqlite"];
    sqlite3 *database;
        ...
        if(sqlite3_prepare_v2(database, [strQuery UTF8String], -1, &statments, NULL)==SQLITE_OK)
        {
            while (sqlite3_step(statments)==SQLITE_ROW)
            {
              ...
              NSString *rtime=[NSString stringWithUTF8String:(char*)sqlite3_column_text(statments, 2)];
                ...
                NSString *raddt=[NSString stringWithUTF8String:(char*)sqlite3_column_text(statments, 4)];
                NSString *recur=[NSString stringWithUTF8String:(char*)sqlite3_column_text(statments, 5)];
    ...
    if( sqlite3_open([strDatabaseFile UTF8String], &database)==SQLITE_OK)
    {
        ...
        if(sqlite3_prepare_v2(database, [strQuery UTF8String], -1, &statments, NULL)==SQLITE_OK)
        {
            while (sqlite3_step(statments)==SQLITE_ROW)
            {
                ...
                NSString *Anm=[NSString stringWithUTF8String:(char*)sqlite3_column_text(statments, 0)];
                
                NSString *Aps=[NSString stringWithUTF8String:(char*)sqlite3_column_text(statments, 1)];
                
                NSDictionary *dic=@{@"NAME":Anm,@"PASS":Aps};
                [arrItems addObject:dic];
            }
    ...
    if( sqlite3_open([strDatabaseFile UTF8String], &database)==SQLITE_OK)
    {
        ...
        if(sqlite3_prepare_v2(database, [strQuery UTF8String], -1, &statments, NULL)==SQLITE_OK)
        {
            while (sqlite3_step(statments)==SQLITE_ROW)
            {
                ...
                NSDictionary *dic=@{@"NAME":unm,@"PASS":ups,@"EMAIL":uEmail,@"CONTACT":[NSNumber numberWithInt:uCon],@"INCOME":[NSNumber numberWithInt:uInc]};
                [arrItems addObject:dic];
            }
